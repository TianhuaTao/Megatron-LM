...
def save_checkpoint(queue, args):
    ...
    def set_hf_layer_state(hf_model, layer_idx, layer_state):
        ...
        # group_query_attention = "num_key_value_heads" in hf_config
        group_query_attention = hasattr(hf_config, "num_key_value_heads")
        nh = hf_config.num_attention_heads
        ng = hf_config.num_key_value_heads if group_query_attention else hf_config.num_attention_heads # no group query attention
        dim = hf_config.hidden_size // nh

        megatron_qkv_weight = megatron_qkv_weight.view(ng, (2 + (nh//ng))*dim, -1)
        q_proj_w_megatron = megatron_qkv_weight[:, :dim*nh//ng, :]
        k_proj_w_megatron = megatron_qkv_weight[:, dim*nh//ng:dim*(nh//ng+1), :]
        v_proj_w_megatron = megatron_qkv_weight[:, dim*(nh//ng+1):, :]
